...
const formFields = [
  ...
  { 
    ...
    type: 'select', 
    options: ['アパート', '一軒家', 'マンション', '戸建て', 'テラスハウス'], 
    required: true,
    example: '例: アパート'
  },
  { 
  ...
  { 
    id: 'size', 
    label: '物件の広さを入力してください（㎡）', 
    type: 'number', 
    placeholder: '広さ', 
    required: true,
    example: '例: 50（半角数字で入力してください）'
  },
  { 
    id: 'max_occupancy', 
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const ownersList = ownersSnapshot.docs.map(doc => ({ id: doc.id, name: doc.data().name, ...doc.data() }));
  ...
  const addProperty = async () => {
    if (Object.values(newProperty).every(field => field)) {
      ...
      try {
        ...
        alert('物件の追加中にエラーが発生しました。もう一度試してください。');
      } finally {
